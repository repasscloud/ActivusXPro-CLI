...
namespace ActivusXPro_CLI.Core.Utilities
{
	public class SearchAD
	{
        ...
        /// <summary>
        /// Prints an ADUser based on the adProperty and the value, invoked by SearchAD.FindADUser
        /// </summary>
        /// <param name="rootDN"></param>
        ...
        /// <summary>
        /// Finds all Disabled ADUsers and publishes report to XLSX format
        /// </summary>
        /// <param name="rootDN"></param>
        ...
        /// <summary>
        /// Finds all Enabled ADUsers and publishes report to XLSX format
        /// </summary>
        /// <param name="rootDN"></param>
        /// <param name="filePath"></param>
        public static void FindAllEnabledADUsers(string rootDN, string filePath)
        {
            using (DirectorySearcher searcher = new DirectorySearcher(new DirectoryEntry(rootDN)))
            {
                searcher.Filter = $"(&(objectClass=user)(!(userAccountControl:1.2.840.113556.1.4.803:=2)))";
                ...
                if (results != null && results.Count > 1)
                {
                    using (var package = new ExcelPackage())
                    {
                        var worksheet = package.Workbook.Worksheets.Add("EnabledUsers");
